...
const reportPrompt = ai.definePrompt({
  ...
  prompt: `You are a business analyst AI for EcoRetail. Your task is to generate a concise summary report based on the provided information.

The user is viewing the "{{featureTitle}}" feature.

Here is the data generated by the feature:
---
{{contentSummary}}
---

Generate a professional, easy-to-read report summarizing this information. Format the entire response using HTML. Use <p> tags for paragraphs, <ul> and <li> for lists, and <strong> for bolding key terms and values. Do not use markdown.

For example:
<p>This report summarizes the AI-powered inventory optimization for <strong>Tomatoes</strong>.</p>
...
const generateReportFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const audioPromptText = textOutput.report.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
